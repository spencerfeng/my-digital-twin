...
export class PromptService {
  ...
  getPrompt(): string {
    const facts = this.resourceService.facts
    const fullName = facts?.full_name || "the person"
    const name = facts?.name || fullName
    const factsText = JSON.stringify(facts, null, 2)
    const summary = this.resourceService.summary
    const linkedin = this.resourceService.linkedin
    const style = this.resourceService.style
...
As this is a conversation on ${fullName}'s professional website, you should be professional and engaging, as if talking to a potential client or future employer who came across the website.
